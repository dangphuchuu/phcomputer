                                    ...
                                    <div class="list-group chat-list" id="chatList" style="max-height: 500px; overflow-y: auto;">
                                        <ul class="list-group list-group-flush">
                                            ...
                                            <li class="list-group-item d-flex align-items-center chat-item">
                                                <img src="https://res.cloudinary.com/{{env('CLOUD_NAME')}}/image/upload/{{$user->image}}.jpg" class="profile_img rounded-circle mr-3" style="width: 40px; height: 40px;" alt="Profile Picture">
                                                <div class="profile_info">
                                                    <span class="profile_name font-weight-bold">{{ $user->firstname }}</span>
                                            ...
                                            <li class="list-group-item d-flex align-items-center chat-item">
                                                ...
                                                @if ($chat->receiver)
                                                <img src="https://res.cloudinary.com/{{env('CLOUD_NAME')}}/image/upload/{{$chat->receiver->image}}.jpg" class="profile_img rounded-circle mr-3" style="width: 40px; height: 40px;" alt="Profile Picture">
                                                <div class="profile_info">
                                                    <span class="profile_name font-weight-bold">{{ $chat->receiver->firstname }}</span>
                                                </div>
                                                @else
                                                <div class="profile_info">
                                                ...
                                                @if ($chat->sender)
                                                <img src="https://res.cloudinary.com/{{env('CLOUD_NAME')}}/image/upload/{{$chat->sender->image}}.jpg" class="profile_img rounded-circle mr-3" style="width: 40px; height: 40px;" alt="Profile Picture">
                                                <div class="profile_info">
                                                    <span class="profile_name font-weight-bold">{{ $chat->sender->firstname }}</span>
                                                </div>
                                                @else
                                                <div class="profile_info">
                                ...
                                <div class="card shadow-sm">
                                    <div class="card-header bg-primary text-white">
                                        <div class="d-flex align-items-center">
                                            ...
                                            <h4 class="mb-0" id="chat_name">{{__("Chatting with")}}</h4>
                                        </div>
                                    </div>
    ...
    let messageHtml = `
        <div class="chat-message">
            <div class="message-avatar">
                <img src="https://res.cloudinary.com/{{env('CLOUD_NAME')}}/image/upload/${senderImage}.jpg" class="rounded-circle" alt="${senderName} Avatar">
            </div>
...
$.ajax({
    ...
    success: function(response) {
        ...
        response.messages.forEach(function(message) {
            ...
            let userAvatar = isSender ? '{{ $LoggedAdminInfo->image }}' : profileImage;
            let userName = isSender ? '{{ $LoggedAdminInfo->firstname }}' : profileName;
            ...
            let imageUrl = getImageUrl(userAvatar); // Use getImageUrl() to handle Cloudinary URLs
...
let cloudName = '{{ env('CLOUD_NAME') }}'; // Get cloud name from .env
let imageUrl = '';
...
$.ajax({
    ...
    success: function(response) {
        if (response.success) {
            ...
            let userAvatar = '{{ $LoggedAdminInfo->image }}';
            let userName = '{{ $LoggedAdminInfo->firstname }}';
